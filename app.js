...
async function assignRolesToTeam(team, users, guild) {
    ...
    if (!gameMembers || !gameMembers["Valorant"]) return;

    console.log(`\n🆕 Xử lý team mới: ${teamName}`);
    await fetchAllMembers(guild);

    const members = gameMembers["Valorant"];
    for (const riotId of members) {
        ...
        let gameRole = await getOrCreateRole(guild, "Val-League Participants", "BLUE");
...
async function updateTeamRoles(team, users, guild) {
    ...
    const oldMembers = prevTeam.gameMembers ? prevTeam.gameMembers["Valorant"] || [] : [];

    if (!gameMembers || !gameMembers["Valorant"]) return;
    const newMembers = gameMembers["Valorant"];
...
const SERVER_URL = `https://mydiscordbot-ugxh.onrender.com/`;
